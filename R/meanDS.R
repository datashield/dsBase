...
meanDS <- function(xvect){
  ...
  #############################################################
  # New proposal for concentration                            #
  #############################################################
  concentration = 0.9
  xtest = xvect/sum(xvect, na.rm = TRUE)
  
  if(any(xtest>concentration, na.rm=TRUE)){
    out.mean <- NA
    stop("FAILED: One value accounts for more than nfilter.conc of the mean", call. = FALSE)
  }
