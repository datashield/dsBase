...
creates 1-dimensional, 2-dimensional and 3-dimensional tables using the
`table` function in native R.
...
- force.nfilter.transmit:
  ...
  for information see help on \<force.nfilter\> argument of `ds.table`.
...
this serverside function is the workhorse of `ds.table` - creating the
table requested in the format specified by `ds.table`. For more
information see help for `ds.table` in DataSHIELD and the `table`
