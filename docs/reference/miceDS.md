...
This function is a wrapper function of the mice from the mice R package.
The function creates multiple imputations (replacement values) for
...
- m:
  ...
  Number of multiple imputations. The default is m=5. The maximum
  allowed number in DataSHIELD is m=20.

- maxit:

  A scalar giving the number of iterations. The default is 5. The
  maximum allowed number in DataSHIELD is maxit=30.
...
post. The function also saves in each server the mids object and all
completed datasets as dataframes.
